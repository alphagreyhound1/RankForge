...
export const Route = createFileRoute("/audit")({
  ...
  head: () => ({
    meta: [
      { title: "Sample Audit Report — RankForge AI" },
      { name: "description", content: "See exactly what a RankForge SEO audit looks like — technical scores, prioritized fixes, revenue impact modeling." },
      { property: "og:title", content: "Sample Audit Report — RankForge AI" },
      { property: "og:description", content: "Editorial-quality SEO audit sample report." },
...
function ScoreCircle({ score }: { score: number }) {
  ...
  return (
    <svg width="220" height="220" viewBox="0 0 220 220" className="rotate-[-90deg]">
      ...
      />
      <text x="110" y="118" textAnchor="middle" transform="rotate(90 110 110)" className="font-mono" style={{ fontSize: 56, fontWeight: 500, fill: "var(--ink)" }}>
        {score}
      </text>
    </svg>
  );
...
const ISSUES = [
  { p: "P0", type: "error", title: "LCP fails on 84% of landing pages", note: "Hero image 2.4MB. Serve as WebP, preload, and use responsive srcset.", impact: "+$8,400 / mo" },
  { p: "P0", type: "error", title: "Missing canonical tags on 342 URLs", note: "Duplicate content dilutes ranking signal across 6 templates.", impact: "+$5,200 / mo" },
  { p: "P1", type: "warning", title: "Schema markup incomplete", note: "Product schema missing on 87% of product pages. No FAQ or Article schema anywhere.", impact: "+$3,100 / mo" },
  { p: "P1", type: "warning", title: "Thin content on 128 category pages", note: "Category pages average 42 words. Competitors average 640 words with topical clusters.", impact: "+$4,600 / mo" },
  { p: "P2", type: "warning", title: "Internal linking bottleneck", note: "128 orphan pages. Homepage funnels 71% of link equity to just 4 URLs.", impact: "+$2,300 / mo" },
  { p: "P2", type: "success", title: "Backlink profile is clean", note: "0 toxic domains, DR 42, natural anchor distribution. Continue current outreach.", impact: "Maintain" },
];
          ...
          <div className="grid grid-cols-1 md:grid-cols-12 gap-8 items-start">
            <div className="md:col-span-8">
              ...
              <p className="mt-6 text-base text-charcoal leading-relaxed max-w-xl">
                A specimen of what our clients receive on day two. 84 diagnostic
                checks, prioritised by revenue impact, delivered as an editorial document.
              </p>
              <div className="mt-8 flex flex-wrap gap-x-8 gap-y-2 label-mono">
                <span>Domain age <span className="text-ink">4.2y</span></span>
                <span>Pages crawled <span className="text-ink">1,284</span></span>
                <span>Keywords tracked <span className="text-ink">2,401</span></span>
                <span>Report date <span className="text-ink">Q2 · 2026</span></span>
              </div>
            </div>
              ...
              <div key={c.name} className="border-r border-b border-border p-6 md:p-8 hover:bg-surface">
                ...
                <div className="mt-6 h-1 bg-border relative overflow-hidden">
                  <div
                    ...
                    style={{ width: `${c.score}%`, background: c.status === "success" ? "var(--success)" : c.status === "warning" ? "var(--warning)" : "var(--error)" }}
                  />
                </div>
      ...
      <section className="border-b border-border bg-surface">
        <div className="mx-auto max-w-[1400px] px-6 md:px-10 py-16 md:py-24">
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-10">
            ...
            <div className="label-mono text-charcoal">Estimated combined lift · <span className="text-success text-lg font-mono">+$23,600 / mo</span></div>
          </div>
          ...
          <div className="border border-border bg-background">
            ...
            {ISSUES.map((issue, i) => (
              <div key={i} className="grid grid-cols-[60px_1fr_120px] md:grid-cols-[80px_60px_1fr_180px] border-b border-border last:border-0 hover:bg-surface cursor-pointer group">
                <div className="p-4 md:p-6 border-r border-border font-mono text-sm text-ink self-center">{issue.p}</div>
                <div className="p-4 md:p-6 border-r border-border hidden md:flex items-center"><StatusIcon type={issue.type} /></div>
                <div className="p-4 md:p-6 border-r border-border">
                  <h3 className="font-serif text-xl md:text-2xl text-ink group-hover:text-accent">{issue.title}</h3>
                  <p className="text-sm text-charcoal mt-2 leading-relaxed">{issue.note}</p>
                </div>
                <div className="p-4 md:p-6 text-right self-center">
                  <div className={`font-mono text-sm md:text-base ${issue.impact === "Maintain" ? "text-charcoal" : "text-success"}`}>{issue.impact}</div>
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-12 gap-8 items-end mb-10">
            ...
            <div className="md:col-span-4 md:col-start-9">
              <p className="text-base text-charcoal leading-relaxed">
                Free, personal, delivered by a strategist — not a PDF generator.
                Drop your URL below.
              </p>
            </div>
          </div>
          <AuditForm />
          <Link to="/contact" className="mt-10 inline-flex items-center gap-2 label-mono link-underline">
